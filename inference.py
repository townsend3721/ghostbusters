...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key in self.keys():
            i = self[key]
            while i != 0:
                weightedKeys.append(key)
                i -= 1
        return random.choice(weightedKeys)



        #self.normalize()
        #return np.random.choice(list(self.keys()), 1, p = list(self.values()))[0]
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for gP in self.allPositions:
            self.beliefs[gP] = self.getObservationProb(observation, gameState.getPacmanPosition(), gP, self.getJailPosition()) * self.beliefs[gP]
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for gP in self.allPositions:
            ...
            for nP in newPosDist:
                tempDist[nP] += newPosDist[nP]* self.beliefs[gP]
